...
for(scenario in c("ssp126", "ssp585")){
  ...
  # dataframe of target months for each region
  regional_months <- data.frame(region = c("southern", "northernmost", "midrange", "kerguelen", "crozet"),
                                months = I(list(c(11,12,1,2,3),
                                                c(6,7,8,9,10),
                                                c(10,11,12,1,2),
                                                c(8,9,10,11,12),
                                                c(8,9,10,11))))
  ...
  # loop over regions
  for(region in regional_months$region){
    print(region)
    
    # get target months
    target_months <- unlist(regional_months$months[regional_months$region == region])
    ...
    foreach(z = 1:8) %dopar% {
      ...
      #-------------------------------------------------------------------
      # Create Rasters for Future Predictions - Southernmost regions first
      #-------------------------------------------------------------------
      ...
      writeRaster(pred_raster, 
                  filename = paste0("penguins/output/climatic model/projections/", scenario, "/", gcm, "/", species, "_", gcm, "_", scenario, "_", region, "_rf_prediction.tif"),
                  overwrite = TRUE)
      ...
      writeRaster(pred_raster, 
                  filename = paste0("penguins/output/climatic model/projections/", scenario, "/", gcm, "/", species, "_", gcm, "_", scenario, "_", region, "_brt_prediction.tif"),
                  overwrite = TRUE)
      ...
      # export the prediction
      writeRaster(pred_raster, 
                  filename = paste0("penguins/output/climatic model/projections/", scenario, "/", gcm, "/", species, "_", gcm, "_", scenario, "_", region, "_gam_prediction.tif"),
                  overwrite = TRUE)
      ...
      # export the prediction
      writeRaster(pred_raster, 
                  filename = paste0("penguins/output/climatic model/projections/", scenario, "/", gcm, "/", species, "_", gcm, "_", scenario, "_", region, "_bart_prediction.tif"),
                  overwrite = TRUE)
      ...
      writeRaster(simple, 
                  filename = paste0("penguins/output/climatic model/projections/", scenario, "/", gcm, "/", species, "_", gcm, "_", scenario, "_", region, "_simple_ensemble.tif"),
                  overwrite = TRUE)
      
      # print gcm completion
      print(paste(gcm, scenario, region))
    }
  }
}