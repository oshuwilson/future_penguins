...
for(scenario in c("ssp126", "ssp585")){
  ...
  foreach(z = 1:8) %dopar% {
    ...
    # read in emperor-specific rasters
    sip <- rast(paste0("Satellite_Data_CMIP/sea_ice_persistence/", scenario, "/future_persistence_", gcm, "_", scenario, ".tif"))
    fast_ice <- rast(paste0("Satellite_Data_CMIP/deltas/fast_ice_area/", gcm, "_", scenario, "_emp_future.tif"))
    dist2coast <- rast("Satellite_Data/static/dist2coast_emp.tif")
    
    # project and resample sea ice persistence
    ...
    sip <- crop(sip, e)
    fast_ice <- crop(fast_ice, e)
    dist2coast <- crop(dist2coast, e)
    
    # make NAs in dist2coast equal max value
    values(dist2coast)[is.na(values(dist2coast))] <- minmax(dist2coast)[2,]
    print("Checkpoint 1")
    ...
    # stack predictors
    stack <- c(temp, prec, now, sip, fast_ice, dist2coast)
    print("Checkpoint 2")
