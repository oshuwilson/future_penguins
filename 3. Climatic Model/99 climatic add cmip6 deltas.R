...
cmip_vars <- c("tas", "pr")
satellite_vars <- c("air_temp", "total_precipitation")
...
for(z in 1:length(cmip_vars)){
  ...
  all_satellite <- rast(paste0("Satellite_Data/monthly/ERA5/", satellite_var, "_monthly.grib"))
  
  #project cmip data to satellite data crs
  delta_126 <- project(delta_126, crs(all_satellite))
  delta_585 <- project(delta_585, crs(all_satellite))
