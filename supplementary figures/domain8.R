#-------------------------------------------------------------------------------
# Evaluate projected species distribution changes in Domain 8
#-------------------------------------------------------------------------------
...
ggsave("text/figures/draft/domainplots/domain8/corehabitat.png", all_grid,
       width = 12, height = 12)
...
domainplot <- ggplot() +
  ...
  geom_spatvector(data = domains %>% filter(Name == "8"), fill = "darkred", col = NA) +
  scale_fill_grey(guide = "none") +
  theme_void()
domainplot + ggview::canvas(width = 6, height = 6)
ggsave("text/figures/draft/domainplots/domain8/domain_highlight.png")
