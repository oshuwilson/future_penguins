#-------------------------------------------------------------------------------
# Evaluate projected species distribution changes around Macquarie Island
#-------------------------------------------------------------------------------
...
# create bounding box around Macquarie Island
this_domain <- ext(145, 170, -58, -50) %>%
  vect(crs = "epsg:4326") %>%
  as.polygons() %>%
  project("epsg:6932")
plot(this_domain)
...
all_grid <- plot_grid(p1_present, p1_future, ncol = 1, rel_heights = c(0.4, 0.6))
all_grid + ggview::canvas(width = 12, height = 12)
ggsave("text/figures/draft/domainplots/domain_macquarie/corehabitat.png", all_grid,
       width = 12, height = 12)
...
ggsave("text/figures/draft/domainplots/domain_macquarie/proposal.png",
       width = 10, height = 7) 
...
domainplot <- ggplot() +
  ...
  geom_spatvector(data = this_domain, fill = "darkred", col = NA) +
  scale_fill_grey(guide = "none") +
  theme_void()
domainplot + ggview::canvas(width = 6, height = 6)
ggsave("text/figures/draft/domainplots/domain_macquarie/domain_highlight.png")
