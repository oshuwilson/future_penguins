...
#------------------------------------------------------
# Repeat for breeding habitat model
#------------------------------------------------------
...
# list files that contain the species, stage, and "pdp"
files <- list.files(path = paste0("output/", model), 
                    pattern = paste0(species, "_pdp_values"), 
                    full.names = TRUE,
                    recursive = TRUE)
...
# rename vars to unify min/max/mean
unique(pdp$var)
pdp <- pdp %>%
...
pdp <- pdp %>%
  mutate(x = case_when(
    var == "temp" ~ x - 273.15, # convert from K to °C
    var == "now" ~ x / 1000, # convert from m to km
    var == "dist2coast" ~ x / 1000, # convert from m to km
    TRUE ~ x
  ))
...
var_names <- c(
  ...
  "now" = "Distance to Nearest Open Water (km)",
  "prec" = "Total Precipitation (m)",
  "dist2coast" = "Distance to Coast (km)",
  "fast_ice" = "Fast Ice Area (km2)", 
  "sip" = "Sea Ice Persistance (days)"
)
...
# export
ggsave(p1, filename = paste0("output/imagery/pdps/", model, "/", species, "_pdp_combined.png"),
       width = 14, height = 8)
