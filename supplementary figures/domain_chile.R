#-------------------------------------------------------------------------------
# Evaluate projected species distribution changes around Falkland Islands
#-------------------------------------------------------------------------------
...
# create bounding box around Chile
this_domain <- ext(-75, -63, -60, -50) %>%
  vect(crs = "epsg:4326") %>%
  as.polygons() %>%
  project("epsg:6932")
plot(this_domain)
...
ggsave("text/figures/draft/domainplots/domain_chile/corehabitat.png", all_grid,
       width = 12, height = 12)
...
proposal <- ggplot() +
  ...
  geom_spatvector(data = mpas, col = NA, fill = "#26547C", alpha = 0.8) +
  geom_spatvector(data = coast, aes(fill = surface, col = surface)) +
  scale_color_manual(values = c("grey40", "grey40"), guide = "none") +
  scale_fill_manual(values = c("grey40", "grey40"), guide = "none") +
  theme_void()
proposal + ggview::canvas(width = 10, height = 7)
ggsave("text/figures/draft/domainplots/domain_chile/proposal.png",
       width = 10, height = 7) 
...
ggsave("text/figures/draft/domainplots/domain_chile/domain_highlight.png", domainplot,
       width = 6, height = 6)
