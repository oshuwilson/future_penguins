...
# create bounding box around falklands Island
this_domain <- ext(-63, -50, -55, -48) %>%
  vect(crs = "epsg:4326") %>%
  as.polygons() %>%
  project("epsg:6932")
plot(this_domain)
...
ggsave("text/figures/draft/domainplots/domain_falklands/corehabitat.png", all_grid,
       width = 12, height = 12)
...
ggsave("text/figures/draft/domainplots/domain_falklands/proposal.png",
       width = 10, height = 7) 
...
ggsave("text/figures/draft/domainplots/domain_falklands/domain_highlight.png", domainplot,
       width = 6, height = 6)
