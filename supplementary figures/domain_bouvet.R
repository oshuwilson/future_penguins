#-------------------------------------------------------------------------------
# Evaluate projected species distribution changes around Bouvet
#-------------------------------------------------------------------------------
...
# create bounding box around Bouvet Island
this_domain <- ext(-5, 10, -60, -50) %>%
  vect(crs = "epsg:4326") %>%
  as.polygons() %>%
  project("epsg:6932")
plot(this_domain)
...
p1_present <- ggplot() +
  ...
  geom_spatvector(data = this_domain, fill = NA, col = NA) +
  theme_void()
p1_present
...
# plot kings macaronis and chinstraps
p1_585 <- ggplot() +
...
ggsave("text/figures/draft/domainplots/domain_bouvet/corehabitat.png", all_grid,
       width = 12, height = 12)
...
ggsave("text/figures/draft/domainplots/domain_bouvet/proposal.png",
       width = 10, height = 7) 
...
ggsave("text/figures/draft/domainplots/domain_bouvet/domain_highlight.png")
