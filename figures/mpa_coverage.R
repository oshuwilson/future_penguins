...
by_mpa_type_notake <- data %>%
  ...
  summarise(area_km2 = sum(mean_area_km2, na.rm = T),
            lower_area_km2 = sum(lower_ci, na.rm = T),
            upper_area_km2 = sum(upper_ci, na.rm = T))
...
# repeat percentages calculations for lower and upper confidence intervals
percentages_ci <- by_mpa_type_notake %>%
  group_by(species, ssp) %>%
  mutate(total_lower_ci = sum(lower_area_km2)) %>%
  mutate(total_upper_ci = sum(upper_area_km2)) %>%
  ungroup() %>%
  mutate(lower_percentage = (lower_area_km2 / total_lower_ci) * 100,
         upper_percentage = (upper_area_km2 / total_upper_ci) * 100) %>%
  mutate(lower_percentage = ifelse(lower_percentage < 0, 0, lower_percentage),
         upper_percentage = ifelse(upper_percentage > 100, 100, upper_percentage))

# add a total protected row for each ssp and species
protected_totals_ci <- percentages_ci %>%
  filter(type %in% c("Existing MPAs: No-Take Zones", "Existing MPAs: Other")) %>%
  group_by(species, ssp) %>%
  summarise(lower_percentage = sum(lower_percentage), upper_percentage = sum(upper_percentage)) %>%
  mutate(type = "Total Protected Areas") %>%
  select(species, ssp, type, lower_percentage, upper_percentage)
...
area <- area %>%
  ...
  mutate(diff = 1 - scaled_area)

# repeat area calculations for lower and upper confidence intervals
area_ci <- data %>%
  group_by(species, ssp) %>%
  summarise(lower_ci = sum(lower_ci), upper_ci = sum(upper_ci)) %>%
  ungroup() %>%
  group_by(species) %>%
  mutate(present_lower_ci = sum(lower_ci[ssp == "Present"])) %>%
  mutate(present_upper_ci = sum(upper_ci[ssp == "Present"])) %>%
  ungroup() %>%
  mutate(scaled_lower_ci = lower_ci / present_lower_ci,
         scaled_upper_ci = upper_ci / present_upper_ci) %>%
  mutate(diff_lower_ci = 1 - scaled_lower_ci,
         diff_upper_ci = 1 - scaled_upper_ci)
...
emp <- emp %>%
  ...
  ungroup() %>%
  mutate(scaled_area = total_area / present_area) %>%
  mutate(diff = 1 - scaled_area)

# average difference across subareas
emp %>%
  group_by(ssp) %>%
