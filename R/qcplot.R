...
##' @param df the tracking dataframe used to fit the model
##' @param device_type either GPS, PTT, or GLS
##' @param step_duration # time step in hours used to fit the model
##' 
...
qcplot <- function(fit, spp, df, device_type, step_duration){

  #extract fitted and predicted values
  ggfit <- grab(fit, what = "fitted")
  ggpred <- grab(fit, what = "predicted")
  
  #remove final underscore and following numbers from id 
  df <- df  %>%
    mutate(new_id = gsub("_\\d+$", "", id))
  
  #separate out individual_id and device_id from the !! separator
  df <- df %>%
    mutate(individual_id = gsub("!!.*", "", new_id),
           device_id = gsub(".*!!", "", new_id))
  
  # get individual_ids and device_ids from data
  individuals <- df %>%
    select(id, individual_id, device_id) %>%
    distinct() 
  
  # join info to fitted tracks
  ...
  #setup pdf export
  pdf(file = paste0(spp, "_", device_type, "_qc_plots.pdf"), width = 8, height = 10, pointsize = 16)
