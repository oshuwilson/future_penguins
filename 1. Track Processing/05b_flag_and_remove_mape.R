...
sp_meta <- meta %>% filter(abbreviated_name %in% c(species_code, "ROPE")) # also include royal penguins
...
ind_flag <- c("MAPE_Heard_2000_10", "MAPE_Heard_2000_8", "MAPE_Heard_2000_9", "Royal_BB_35463")

# define corresponding device IDs flagged for removal - MUST BE SAME ORDER AS INDIVIDUAL IDs
dev_flag <- c("MAPE_Heard_2000_10", "MAPE_Heard_2000_8", "MAPE_Heard_2000_9", "35463")
...
meta <- meta %>%
  filter(!abbreviated_name %in% c(species_code, "ROPE")) %>%
